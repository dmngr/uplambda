...
if (args.v || args.version) {
  exec(
    "npm show @deliverymanager/uplambda version",
  ...
  new Promise(function (resolve, reject) {
    fs.readFile(homedir + "/.uplambda.json", "utf-8", function (err, data) {
      if (err) {
        if (err.code !== "ENOENT") reject(err);
          ...
          reject(
            `Config file ${homedir}/.uplambda.json was not found. Run "uplambda --account --add" to init`
          );
    ...
    .then((config) => {
      ...
      if (args.account && !args.use) {
        if (!args.add && !args.delete && !args.list)
          return Promise.reject(
            "When updating accounts, use --add to add/update account, and --delete to delete an account"
          );
        if (args.list) {
          ...
          if (!config[args.delete])
            return Promise.reject(
              `Account: "${
                args.delete === true ? undefined : args.delete
              }" not found`
            );
          else {
            delete config[args.delete];
            return fs.writeFileSync(
              homedir + "/.uplambda.json",
              JSON.stringify(config, null, 2)
            );
          }
        } else {
          ...
          return prompt(
            "account alias/name(any name, no spaces. e.g. my_project_account): "
          )
            .then((res) => {
              if (!res || !res.match(/^\w+$/))
                return Promise.reject(
                  "Invalid account name. Must satisfy /^\\w+$/"
                );
              ...
              return prompt(
                "account arn(region:account_number, e.g. eu-west-1:1234567890): "
              );
            })
            .then((res) => {
              if (
                !config[account_name].account &&
                (!res || !res.match(/^[a-z]{2}-[a-z0-9]+-\d{1,2}:\d+$/))
              )
                return Promise.reject(
                  "Invalid account arn, must match /^[a-z]{2}-[a-z0-9]+-\\d{1, 2}:\\d+$/."
                );
            ...
            .then((res) => {
              if (!config[account_name].aws_access_key_id && !res)
                return Promise.reject("Invalid aws_access_key_id");

              if (res) config[account_name].aws_access_key_id = res;
              return prompt("aws_secret_access_key: ");
            })
            .then((res) => {
              if (!config[account_name].aws_secret_access_key && !res)
                return Promise.reject("Invalid aws_secret_access_key");
            ...
            .then((res) => {
              ...
              return prompt(
                "Set S3 bucket used with cloudformation templates to update lambda code (Optional): "
              );
            })
            .then((res) => {
              ...
              return prompt(
                "Set lambda IAM role name (Optional, used with cloudformation): "
              );
            })
            .then((res) => {
        ...
        if (!config[args.use])
          return Promise.reject(
            `Account: "${args.use}" not found. Run "uplambda --account --add"`
          );
        ...
        //WE ARE IN THE LAMBDA LAYER UPLOAD
        ...
        if (tmp_accounts.length === 0)
          return Promise.reject(
            `Invalid ${homedir}/.uplambda.json. At least one account must be active at a time. Run "uplambda --account --use <your_account_alias>" to choose which account to enable`
          );
        if (tmp_accounts.length !== 1)
          return Promise.reject(
            `Invalid ${homedir}/.uplambda.json. Only one account can be active at a time. Run "uplambda --account --use <your_account_alias>" to choose which account to enable`
          );
        else {
          ...
          new Promise(function (resolve, reject) {
            ...
            depcheck(process.cwd(), {}, (unused) => {
              try {
                ...
                const deps = Object.keys(unused.missing).filter(
                  (dep) =>
                    dep !== "aws-sdk" &&
                    dep !== "crypto" &&
                    dep !== "@sparticuz/chromium"
                );
                if (deps.length > 0) {
                  deps.forEach((key) => {
                    ...
                    unused.missing[key].forEach((path) => {
                      console.log(
                        path.substring(
                          path.indexOf(process.cwd()) + process.cwd().length + 1
                        )
                      );
                    });
                  });
            ...
            .then(() => {
              if (package_json.dependencies) {
                return Promise.all(
                  Object.keys(package_json.dependencies).map((dep) => {
                    try {
                      console.log("dep:", dep);
                      if (
                        dep == "aws-sdk" ||
                        ...
                        if (
                          fs.existsSync(process.cwd() + "/node_modules/" + dep)
                        ) {
                          return Promise.resolve();
                        }
            ...
            .then(() => {
              if (!package_json.files) return Promise.resolve();
                ...
                const missing = _.find(
                  package_json.files,
                  (filename) =>
                    !fs.existsSync(path.join(process.cwd(), filename))
                );
                return missing
                  ? Promise.reject(`File: ${missing} is missing`)
                  : Promise.resolve();
              }
            })
            .then(() => getApiInfo())
            ...
            .then((_api_info) => {
              ...
              else if (!api_info.stageNames || api_info.stageNames.length === 0)
            ...
            .then((aliasVerified) => {
              if (aliasVerified)
                ...
                if (args.publish)
                  return Promise.reject(
                    "Alias should be the name of the current branch"
                  );
                else
                  console.log(
                    colors.red("Alias should be the name of the current branch")
                  );
              }
            ...
            .then((_functionName) => {
              ...
              return new Promise(function (resolve, reject) {
                exec(
                  `rm -f ${homedir}/${localPath}/${functionName}.zip; zip -r ${homedir}/${localPath}/${functionName}.zip . -x *.git*`,
                  {
                    maxBuffer: 1e8,
                  },
                  function (err, stdout, stderr) {
            ...
            .then(() => {
              ...
              if (
                fs.existsSync("cloudformation") &&
                fs.existsSync("cloudformation/stack.json")
              )
                stack = fs.readFileSync("cloudformation/stack.json", "utf-8");

              try {
                private_params = JSON.parse(
                  fs.readFileSync("cloudformation/params.json")
                );
              } catch (e) {
            ...
            .then((zip) => {
              if (args.cloudformation) {
                ...
                console.log(
                  `Uploading ${args.publish ? alias : "$LATEST"} to s3..`
                );
                return uploadS3(
                  functionName,
                  zip,
                      ...
                      if (args.publish) {
                        if (
                          (!api_info ||
                            ...
                            !api_info.method ||
                            !api_info.stageNames ||
                            api_info.stageNames.length === 0 ||
                            !alias) &&
                          !package_json.no_api
                        )
                          return Promise.reject("Invalid api/alias info");
                        else {
                          ...
                          return updateAlias(
                            ...
                            .then(() => {
                              console.log(
                                colors.blue("Current Branch/Lambda Alias:"),
                                colors.green(alias)
                              );
                              if (otherBranches.length > 0) {
                                ...
                                otherBranches.forEach(function (branchName) {
                                  if (
                                    branchName[0] == branchName[0].toUpperCase()
                                  )
                ...
                console.log(
                  `Uploading to ${
                    args.publish ? colors.cyan(alias) : colors.cyan("$LATEST")
                  }..`
                );
                ...
                return getStageVariable(
                  ...
                  if (stageVariable !== "dev") {
                    console.log(
                      "\n" +
                        colors.red(
                          `Stage Variable in API GW Stage 'dev':${stageVariable}`
                        )
                    );
                    console.log(
                      "\n" +
                        colors.red(
                          `For tests on front end, set Stage Variable with name '${functionName}' equal to 'dev' in API GW, stage 'dev'`
                        )
                    );
                  } else
                    console.log(
    ...
    .then(() => {
      ...
      if (args.logs)
        exec(`awslogs get /aws/lambda/${functionName} --watch`).stdout.pipe(
          process.stdout
        );
      else process.exit();
    })
